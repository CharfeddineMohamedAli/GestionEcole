 ...
 */
public class FXMLAdminButtController implements Initializable {
    ...
    public static void setNote_transport(Absence absence_transport) {
        FXMLinterfaceController.absence_transport = absence_transport;
    }
    ...
    private void consult(ActionEvent event) throws IOException, SQLException {
        absence_transport.setId_classe(ns.findidclassebynum(Integer.parseInt(combobox.getSelectionModel().getSelectedItem())));
        absence_transport.setId_mat(ns.findidmatierebymatiere(mati1.getSelectionModel().getSelectedItem().toString()));

        AnchorPane newLoadedPane = FXMLLoader.load(getClass().getResource("/FXML/AdminButtConsMove.fxml"));
