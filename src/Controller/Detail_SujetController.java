...
public class Detail_SujetController implements Initializable {
    ...
    public void initialize(URL url, ResourceBundle rb) {
        ...
        try {
            ...
            if (AffichageForumController.getSujet_à_ouvrir().getId_User() == UserUniTech.userConnecte.getId_user()) {
                id_jaime.setVisible(false);
                id_jaimepas.setVisible(false);
            } else {
                if (jaimeService.FindValeurJaimeByIdUserAndIdSujet(UserUniTech.userConnecte.getId_user(), AffichageForumController.getSujet_à_ouvrir().getId_Sujet()) == 0) {
                    id_jaime.setVisible(true);
                    id_jaimepas.setVisible(false);
                } else if (jaimeService.FindValeurJaimeByIdUserAndIdSujet(UserUniTech.userConnecte.getId_user(), AffichageForumController.getSujet_à_ouvrir().getId_Sujet()) == 1) {
                    id_jaime.setVisible(true);
                    id_jaimepas.setVisible(false);
                } else {
                    id_jaime.setVisible(false);
    ...
    private void afficher_liste_commentaires() throws SQLException {
        ...
        ff.forEach(e -> {
            ...
            if (e.getId_User() == UserUniTech.userConnecte.getId_user()) {
...
//textField[i].getStyleClass().add("recumsg");
    ...
    void ajoutComment(ActionEvent event) throws SQLException {
        Commentaire commentaire = new Commentaire(AffichageForumController.getSujet_à_ouvrir().getId_Sujet(), UserUniTech.userConnecte.getId_user(), text_comment.getText());
    ...
    void jaime(ActionEvent event) throws SQLException {
        ...
        if (jaimeService.FindValeurJaimeByIdUserAndIdSujet(UserUniTech.userConnecte.getId_user(), AffichageForumController.getSujet_à_ouvrir().getId_Sujet()) == 0) {
            jaimeService.ajouterJaime(new Jaime(UserUniTech.userConnecte.getId_user(), AffichageForumController.getSujet_à_ouvrir().getId_Sujet(), 2));

        } else if (jaimeService.FindValeurJaimeByIdUserAndIdSujet(UserUniTech.userConnecte.getId_user(), AffichageForumController.getSujet_à_ouvrir().getId_Sujet()) == 1) {
            jaimeService.incrementerjaime(UserUniTech.userConnecte.getId_user(), AffichageForumController.getSujet_à_ouvrir().getId_Sujet());
        }
        id_nbre_jaime.setText(String.valueOf(sujetService.FindNombreJaimeById_Sujet(AffichageForumController.getSujet_à_ouvrir().getId_Sujet())));
    ...
    void jaimepas(ActionEvent event) throws SQLException {
        jaimeService.decrementerjaime(UserUniTech.userConnecte.getId_user(), AffichageForumController.getSujet_à_ouvrir().getId_Sujet());
